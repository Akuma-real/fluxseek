...
Future<String?> _readLineViaShell(String prompt) async {
  ...
  try {
    replyFile = File(
      ...
      'fluxseek_cli_prompt_${pid}_${DateTime.now().microsecondsSinceEpoch}.txt',
    );
    final environment = Map<String, String>.from(Platform.environment)
      ..['FLUXSEEK_CLI_PROMPT'] = prompt
      ..['FLUXSEEK_CLI_REPLY_FILE'] = replyFile.path;
    ...
    final arguments = Platform.isWindows
        ? const [
            ...
            '-NoProfile',
            '-Command',
            r"$reply = Read-Host $env:FLUXSEEK_CLI_PROMPT; if ($null -eq $reply) { exit 2 }; Set-Content -LiteralPath $env:FLUXSEEK_CLI_REPLY_FILE -NoNewline -Value $reply; exit 0",
          ]
        : [
            '-c',
            r'printf "%s" "$FLUXSEEK_CLI_PROMPT" > /dev/tty; '
                r'IFS= read -r reply < /dev/tty || exit 2; '
                r'printf "%s" "$reply" > "$FLUXSEEK_CLI_REPLY_FILE"',
          ];
...
bool _forcePlainUi() {
  final value = Platform.environment['FLUXSEEK_CLI_FORCE_PLAIN']?.trim().toLowerCase();
  return value == '1' || value == 'true' || value == 'yes';
}

List<String> _loadScriptedPromptAnswers() {
  final raw =
      Platform.environment['FLUXSEEK_CLI_TEST_INPUTS']?.trim() ??
      Platform.environment['FLUXSEEK_RELEASE_TEST_INPUTS']?.trim();
