...
/// NodeSeek 客户端 Provider
final nodeSeekClientProvider = Provider((ref) => NodeSeekClient());
...
class CurrentUserNotifier extends AsyncNotifier<User?> {
  ...
  Future<User?> _loadUserWithCache(NodeSeekClient service) async {
  ...
  Future<User?> _loadUser(NodeSeekClient service) async {
  ...
  void _refreshUser(NodeSeekClient service, User preloadedUser) {
    Future(() async {
      try {
        final user = await service.getCurrentUser();
        if (user == null) return;
