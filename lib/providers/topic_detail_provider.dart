...
class TopicDetailNotifier extends AsyncNotifier<TopicDetail> {
  ...
  /// 待加载的新帖子 ID 队列（对齐网页端 _newPostsInStream）
  ...
  int _nodeSeekSyntheticPostId(int postNumber) {
    return -((arg.topicId * 1000000) + postNumber);
  }

  List<int> _buildNodeSeekMergedStream(
    TopicDetail detail,
    List<Post> mergedPosts,
  ) {
    var totalPosts = detail.postsCount;
    for (final post in mergedPosts) {
      if (post.postNumber > totalPosts) totalPosts = post.postNumber;
    }
    final postIdByNumber = <int, int>{
      for (final post in mergedPosts) post.postNumber: post.id,
    };
    return [
      for (var postNumber = 1; postNumber <= totalPosts; postNumber++)
        postIdByNumber[postNumber] ?? _nodeSeekSyntheticPostId(postNumber),
    ];
  }
