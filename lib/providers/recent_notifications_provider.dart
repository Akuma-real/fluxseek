...
class RecentNotificationsNotifier
    extends AsyncNotifier<List<NodeSeekNotification>> {
  @override
  Future<List<NodeSeekNotification>> build() async {
  ...
  /// 高优先级未读通知排在最前
  void addNotification(NodeSeekNotification notification) {
    ...
    final newList = List<NodeSeekNotification>.from(currentList);
  ...
  /// 沿用网页端 逻辑：更新已读状态 + 移除不在 recent 中的过期通知
  void syncWithRecent(Map<int, bool> readStatusMap) {
    final currentList = state.value;
    if (currentList == null) return;

    // 与网页端对齐：只保留 recent 中存在的通知，同时更新已读状态
    final newList = <NodeSeekNotification>[];
