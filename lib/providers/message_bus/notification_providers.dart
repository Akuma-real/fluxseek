...
/// 通知频道监听器
/// 当收到通知消息时更新计数并刷新通知列表
/// 沿用网页端 逻辑：通过增量更新（插入新通知 + recent 同步已读状态）保持列表同步
          ...
          if (recentState.hasValue) {
            ...
            if (lastNotification is Map<String, dynamic>) {
              ...
              if (notification is Map<String, dynamic>) {
                try {
                  final newNotification = NodeSeekNotification.fromJson(
                    notification,
                  );
                  debugPrint(
...
class NotificationAlertChannelNotifier extends Notifier<void> {
  ...
  void build() {
    ...
    // 网页端使用 /notification-alert/{userId} 频道触发桌面通知
