...
import '../nodeseek_providers.dart';
...
final topicTrackingStateMetaProvider = FutureProvider<Map<String, dynamic>?>((
  ...
  final service = ref.watch(nodeSeekClientProvider);
  return service.getPreloadedTopicTrackingMeta();
});

// ─── 话题追踪状态（对齐网页端 网页版 topic-tracking-state.js）───
...
class TrackedTopicState {
  ...
  /// 从预加载数据的 JSON 构建
  ///
  /// 注意：预加载数据中没有 created_in_new_period 字段，
...
/// 全局话题追踪状态 Notifier
/// 对齐网页端 网页版的 topic-tracking-state.js
class TopicTrackingStateNotifier extends Notifier<Map<int, TrackedTopicState>> {
  ...
  /// 对齐网页端 JS topic-tracking-state.js 的 _processChannelPayload
...
class MessageBusInitNotifier extends Notifier<void> {
  ...
  void build() {
    ...
    if (_isOfficialNodeSeekHost) {
      messageBus.stopAll();
      debugPrint('[MessageBusInit] NodeSeek 非旧论坛接口，跳过 MessageBus');
      return;
    }

    // 对齐网页端：long_polling_base_url 对匿名和登录用户都生效，
...
/// 话题列表频道监听器（对齐网页端 网页版 TopicTrackingState）
///
...
class LatestChannelNotifier extends Notifier<TopicListIncomingState> {
  ...
  TopicListIncomingState build() {
    ...
    if (_isOfficialNodeSeekHost) {
      debugPrint('[LatestChannel] NodeSeek 非旧论坛接口，跳过 MessageBus 订阅');
