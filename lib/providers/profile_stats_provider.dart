...
class ProfileStatsConfigNotifier extends Notifier<ProfileStatsConfig> {
  ...
  ProfileStatsConfig build() {
    ...
    if (jsonStr != null) {
      try {
        ...
        return _normalizeNodeSeekConfig(config);
      } catch (_) {
        // 配置损坏，使用默认值
      }
    }
  ...
  void update(ProfileStatsConfig config) {
    final normalized = _normalizeNodeSeekConfig(config);
    state = normalized;
    _save(normalized);
  }

  ProfileStatsConfig _normalizeNodeSeekConfig(ProfileStatsConfig config) {
    if (!config.enabledStats.contains(ProfileStatType.postsReadCount)) {
      return config;
    }
