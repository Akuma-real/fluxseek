...
extension LoadingMethods on TopicDetailNotifier {
  ...
  Future<void> _loadPreviousNodeSeek() async {
    _isLoadingPrevious = true;
    try {
      state = const AsyncLoading<TopicDetail>().copyWithPrevious(state);
      ...
      final result = await AsyncValue.guard(() async {
        ...
        final newPostStream = await service.getPostsByNumber(
          arg.topicId,
          postNumber: currentPosts.first.postNumber,
          asc: false,
        );
        final existingIds = currentPosts.map((p) => p.id).toSet();
        final newPosts = newPostStream.posts
        ...
        final mergedStream = _buildNodeSeekMergedStream(
          currentDetail,
          mergedPosts,
        );
  ...
  Future<void> _loadMoreNodeSeek() async {
    _isLoadingMore = true;
    try {
      state = const AsyncLoading<TopicDetail>().copyWithPrevious(state);
      ...
      final result = await AsyncValue.guard(() async {
        ...
        final newPostStream = await service.getPostsByNumber(
          arg.topicId,
          postNumber: currentPosts.last.postNumber,
          asc: true,
        );
        final existingIds = currentPosts.map((p) => p.id).toSet();
        final newPosts = newPostStream.posts
  ...
  /// 对齐网页端：不在底部时只更新 stream，在底部时批量加载帖子内容
  void onNewPostCreated(int postId) {
    ...
    // 对齐网页端 triggerNewPostsInStream：先基于旧边界判断是否已加载到底部，
    ...
    // 对齐网页端 loadedAllPosts：收到新帖前已加载到底部时才批量拉取新帖子内容，
  ...
  /// 批量加载待处理的新帖子（对齐网页端 triggerNewPostsInStream）
  Future<void> _loadPendingNewPosts() async {
    ...
    try {
      ...
      // 本地递增被回复帖子的 replyCount（与 网页端做法一致）
  ...
  Future<int> loadPostNumber(int postNumber) async {
    ...
    try {
      ...
      if (_isOfficialNodeSeekHost) {
        ...
        _updateBoundaryState(newPosts, newDetail.postStream.stream);
        if (!ref.mounted) return -1;
        state = AsyncValue.data(newDetail);
        return newPosts.indexWhere((p) => p.postNumber == postNumber);
      }
