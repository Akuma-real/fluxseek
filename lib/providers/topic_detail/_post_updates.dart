...
extension PostUpdateMethods on TopicDetailNotifier {
  ...
  /// 对齐网页端 官方 triggerChangedPost：同一 postId 同时只有一个在途请求，
  ...
  /// 与 网页端一致，使用 /posts/{id}.json 单帖接口获取完整数据
  ...
  void _applyPostUpdate(
    ...
    final finalPost = preserveCooked
        ? updatedPost.copyWith(cooked: oldPost.cooked, read: oldPost.read)
        : updatedPost;
  ...
  /// 更新单个帖子的点赞状态
  void applyLikeToggle(int postId, {required bool liked, required int likeCount}) {
    _updatePostById(
      postId,
      (post) => post.copyWith(liked: liked, likeCount: likeCount),
    );
  }

  /// 更新单个帖子的踩状态
  void applyDislikeToggle(int postId, {required bool disliked, required int dislikeCount}) {
    _updatePostById(
      postId,
      (post) => post.copyWith(disliked: disliked, dislikeCount: dislikeCount),
    );
  }

  /// 更新单个帖子的鸡腿状态
  void applyUpvoteToggle(int postId, {required bool upvoted, required int upvoteCount}) {
    _updatePostById(
      postId,
      (post) => post.copyWith(upvoted: upvoted, upvoteCount: upvoteCount),
    );
  }
