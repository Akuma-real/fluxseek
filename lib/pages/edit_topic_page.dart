...
import 'package:fluxseek/widgets/common/loading_spinner.dart';
import 'package:fluxseek/widgets/markdown_editor/markdown_editor.dart';
import 'package:fluxseek/models/category.dart';
import 'package:fluxseek/models/topic.dart';

import 'package:dio/dio.dart';
import 'package:fluxseek/providers/nodeseek_providers.dart';
import 'package:fluxseek/services/app_error_handler.dart';
import 'package:fluxseek/services/nodeseek/node_seek_service.dart';
import 'package:fluxseek/services/toast_service.dart';
import 'package:fluxseek/widgets/markdown_editor/markdown_renderer.dart';
import 'package:fluxseek/widgets/topic/topic_editor_helpers.dart';
import '../l10n/s.dart';
...
class _EditTopicPageState extends ConsumerState<EditTopicPage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // 更新话题元数据（如果有变化且有权限）
      // NodeSeek: title + content 通过一次 edit-discussion 调用完成；
      // 旧 Discourse 兼容保留分开调用。
      final isNodeSeek = NodeSeekService.isOfficialHost;
      final needsNodeSeekTopicEdit =
          isNodeSeek && (titleChanged || contentChanged);

      if (!isNodeSeek && (titleChanged || categoryChanged || tagsChanged)) {
      ...
      // 更新首贴内容（如果有变化且有权限）
      Post? updatedPost;
      if (needsNodeSeekTopicEdit && _firstPost != null) {
        // NodeSeek: 一次调用同时更新标题和内容
        updatedPost = await service.updatePost(
          postId: widget.topicDetail.id,
          raw: contentChanged ? newContent : (_originalContent ?? newContent),
          isTopic: true,
          title: titleChanged ? newTitle : (_originalTitle ?? newTitle),
        );
      } else if (contentChanged && _firstPost != null) {
        updatedPost = await service.updatePost(
