...
extension _ScrollActions on _TopicDetailPageState {
  ...
  void _updateStreamIndexForPostNumber(int postNumber) {
    ...
    if (Uri.tryParse(AppConstants.baseUrl)?.host == 'www.nodeseek.com') {
      _controller.updateStreamIndex(postNumber.clamp(1, stream.length));
      return;
    }
  ...
  void _selectShortcutPostNumber(TopicDetail detail, int postNumber) {
    ...
    if (Uri.tryParse(AppConstants.baseUrl)?.host == 'www.nodeseek.com') {
      _controller.updateStreamIndex(
        postNumber.clamp(1, detail.postStream.stream.length),
      );
      return;
    }
  ...
  Future<void> _scrollToPostById(int postId) async {
    ...
    if (Uri.tryParse(AppConstants.baseUrl)?.host == 'www.nodeseek.com' &&
        postId < 0) {
      final postNumber = (-postId) % 1000000;
      if (postNumber > 0) {
