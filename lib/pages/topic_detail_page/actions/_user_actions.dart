...
extension _UserActions on _TopicDetailPageState {
  ...
  Future<void> _handleReply(Post? replyToPost) async {
    ...
    final preloadedDraftFuture = NodeSeekClient().getDraft(draftKey);
  ...
  void _shareTopic() {
    ...
    final url = ShareUtils.buildShareUrl(
      path: ShareUtils.topicPath(widget.topicId),
      username: username,
  ...
  Future<void> _handleQuotePost(Post post) async {
    ...
    String markdown = '';
    final raw = await NodeSeekClient().getPostRaw(post.id);
  ...
  Future<void> _handlePostBookmark(Post post) async {
    ...
    try {
      final bookmarkId = await NodeSeekClient().bookmarkPost(post.id);
      if (!mounted) return;
  ...
  void _sharePost(Post post) {
    ...
    final prefs = ref.read(preferencesProvider);
    final url = ShareUtils.buildShareUrl(
      path: ShareUtils.topicPath(widget.topicId, postNumber: post.postNumber),
      username: username,
      anonymousShare: prefs.anonymousShare,
    );
    SharePlus.instance.share(ShareParams(text: url));
  ...
  Future<void> _handleDeletePost(Post post) async {
    ...
    try {
      // NodeSeek 楼主帖（postNumber == 1）需要用 delete-discussion
      await NodeSeekClient().deletePost(
        post.id,
        isTopic: post.postNumber == 1,
      );
      if (!mounted) return;
  ...
  Future<void> _handleNestedCreated(int postId, int? userId) async {
    ...
    try {
      final post = await NodeSeekClient().getPost(postId);
      if (!mounted) return;
  ...
  void _handlePostUpdate(TopicDetailNotifier notifier, PostUpdate update) {
    switch (update.type) {
      ...
      case TopicMessageType.acted:
        // 对齐网页端 官方 triggerChangedPost：acted 也传 updatedAt 做去重
