...
import '../widgets/content/html_content/html_content_widget.dart';
...
class UserProfilePage extends ConsumerStatefulWidget {
  ...
  const UserProfilePage({super.key, required this.username, this.uid});
...
class _UserProfilePageState extends ConsumerState<UserProfilePage>
  ...
  // tab 对应的 filter: summary=概况, 4=主题帖, 5=评论
  static const List<String> _tabFilters = ['summary', '4', '5'];
  ...
  Future<void> _loadUser() async {
    try {
      ...
      final results = uid == null
          ...
          : await Future.wait([
              service.getUserByUid(uid),
              service.getUserSummaryByUid(uid),
            ]);
  ...
  void _shareUser() {
    ...
    final url = ShareUtils.buildShareUrl(
      path: widget.uid == null
          ? '/member?q=${widget.username}'
          : '/space/${widget.uid}',
      username: username,
  ...
  Future<String?> _showIgnoreDurationPicker() async {
    // 与 网页端 extendedDefaultTimeShortcuts 保持一致
  ...
  Future<void> _loadActions(String filter, {bool loadMore = false}) async {
    ...
    try {
      ...
      final response = widget.uid == null
          ? await service.getUserActions(
              widget.username,
              filter: filter,
              offset: offset,
            )
          : await service.getUserActionsByUid(
              widget.uid!,
              filter: filter,
              offset: offset,
            );
  ...
  Widget _buildSliverAppBar(
    ...
    return SliverAppBar(
      ...
      actions: [
        ...
        if (_user != null && _user!.canSendPrivateMessageToUser == true)
          IconButton(
        ...
        SwipeDismissiblePopupMenuButton<String>(
          ...
          itemBuilder: (context) {
            ...
            return [
              ...
              if (!isOwnProfile &&
                  _user != null &&
                  (_user!.canMuteUser == true ||
                      _user!.canIgnoreUser == true)) ...[
        ...
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          return Stack(
            ...
            children: [
              ...
              if (hasBackground)
                Image(
                  image: appImageProvider(UrlHelper.resolveUrlWithCdn(bgUrl)),
                  fit: BoxFit.cover,
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Wrap(
                              ...
                              children: [
                                _buildStatSlot(
                                  _summary!.daysVisited.toString(),
                                  '加入天数',
                                  _summary!.daysVisited,
                                ),
                                _buildStatSlot(
                                  NumberUtils.formatCount(_user!.trustLevel),
                                  '等级',
                                  _user!.trustLevel,
                                ),
                                _buildStatSlot(
                                  (_user!.gamificationScore ??
  ...
  Widget _buildSummaryTab() {
    ...
    return ListView(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
      children: [
        _buildNodeSeekOverview(theme, summary),
  ...
  Widget _buildNodeSeekOverview(ThemeData theme, UserSummary summary) {
    final metrics = [
      (
      ...
      (
        ...
        label: '等级',
        value: NumberUtils.formatCount(_user?.trustLevel ?? 0),
      ),
      (
        icon: Icons.restaurant_menu_rounded,
        label: '鸡腿数目',
        value: (_user?.gamificationScore ?? summary.likesReceived).toString(),
      ),
      (
        icon: Icons.edit_outlined,
        label: '主题帖数',
        value: NumberUtils.formatCount(summary.topicCount),
      ),
      (
        icon: Icons.forum_outlined,
        label: '评论数目',
        value: NumberUtils.formatCount(summary.postCount),
      ),
    ];
          ...
          itemBuilder: (context, index) {
            ...
            return Container(
              ...
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Icon(metric.icon, size: 24, color: theme.colorScheme.primary),
                  const SizedBox(height: 8),
                  Text(
                    metric.label,
                    maxLines: 1,
