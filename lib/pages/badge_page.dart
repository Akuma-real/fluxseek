...
class _BadgePageState extends ConsumerState<BadgePage> {
  final NodeSeekClient _service = NodeSeekClient();
  ...
  Widget _buildLargeBadgeIcon(Badge badge, Color color) {
    if (badge.imageUrl != null && badge.imageUrl!.isNotEmpty) {
      return Image(
        image: appImageProvider(UrlHelper.resolveUrlWithCdn(badge.imageUrl!)),
        width: 100,
        height: 100,
        fit: BoxFit.contain,
...
class _UserBadgeItem extends StatelessWidget {
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: CircleAvatar(
                  ...
                  backgroundImage: appImageProvider(user.getAvatarUrl()),
                ),
              ),
            ),
  ...
  void _navigateToUser(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => UserProfilePage(username: user.username, uid: user.id),
      ),
    );
  }
