...
class _PreviewSectionState extends ConsumerState<_PreviewSection> {
  ...
  void initState() {
    super.initState();
    _effectiveSource = _normalizeSource(widget.config.dataSource);
  }
  ...
  @override
  Widget build(BuildContext context) {
    final displayConfig = widget.config.copyWith(
      dataSource: _normalizeSource(_effectiveSource),
    );
  ...
  StatsDataSource _normalizeSource(StatsDataSource source) {
    if (source == StatsDataSource.connect &&
        !AppConstants.siteFeatures.connectStats) {
      return StatsDataSource.summary;
    }
    return source;
  }
  ...
  AsyncValue _watchSourceState(WidgetRef ref) {
    switch (_effectiveSource) {
      ...
      case StatsDataSource.connect:
        if (!AppConstants.siteFeatures.connectStats) {
          return ref.watch(userSummaryProvider);
        }
...
class _DataSourceSelector extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final notifier = ref.read(profileStatsConfigProvider.notifier);
    final sources = StatsDataSource.values.where(
      (source) =>
...
class _StatChipGrid extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final unselected = ProfileStatType.values
        .where(
          (s) =>
              !selected.contains(s) && isStatCompatible(s, config.dataSource),
        )
        .toList();
      ...
      children: [
        ...
        for (final stat in unselected)
          FilterChip(
            label: Text(getStatLabel(stat)),
            selected: false,
            onSelected: (_) => notifier.addStat(stat),
            showCheckmark: false,
            materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
            visualDensity: VisualDensity.compact,
          ),
