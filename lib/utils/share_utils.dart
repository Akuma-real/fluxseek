...
class ShareUtils {
  /// 构建分享链接
  ///
  /// [path] 路径部分，如 `/post-123-1` 或 `/space/456`
  /// [username] 当前用户名
  ...
  static int topicPageForPostNumber(int postNumber) {
    if (postNumber <= 1) return 1;
    return ((postNumber - 1) ~/ 10) + 1;
  }

  static String topicPath(int topicId, {int postNumber = 1}) {
    return '/post-$topicId-${topicPageForPostNumber(postNumber)}';
  }

  static String topicUrl(int topicId, {int postNumber = 1}) {
    return '${AppConstants.baseUrl}${topicPath(topicId, postNumber: postNumber)}';
  }

  /// 分享或保存文件
  ///
  /// 桌面端弹出"另存为"对话框，移动端使用系统分享面板
  static Future<void> shareOrSaveFile(XFile file, {String? subject}) async {
