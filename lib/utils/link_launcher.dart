...
import 'node_seek_url_parser.dart';
import 'link_security.dart';
import 'url_helper.dart';

const _browserChannel = MethodChannel('com.akuma.fluxseek/browser');
...
bool isInternalUrl(Uri uri) {
  ...
  final baseHost = baseUri.host; // 如 'www.nodeseek.com'
  final host = uri.host;
...
/// - 话题链接 /post-123-1 → 调用 onInternalLinkTap 或用 WebView 打开
...
Future<void> launchContentLink(
  ...
  if (url.startsWith('upload://')) {
    url = await NodeSeekClient().resolveShortUrlForLink(url) ?? url;
    if (!context.mounted) return;
  }

  // 1. 识别用户链接 /u/username
  final userInfo = NodeSeekUrlParser.parseUser(url);
  ...
  // 2. 解析话题链接
  final topicInfo = NodeSeekUrlParser.parseTopic(url);
