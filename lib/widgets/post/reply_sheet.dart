...
class _ReplySheetState extends ConsumerState<ReplySheet> {
  ...
  Future<void> _loadPostRaw() async {
    ...
    try {
      final raw = await NodeSeekClient().getPostRaw(widget.editPost!.id);
      if (mounted && raw != null) {
  ...
  Future<void> _submit() async {
    ...
    try {
      if (_isEditMode) {
        // 编辑模式：更新帖子
        final updatedPost = await NodeSeekClient().updatePost(
        ...
        await NodeSeekClient().createPrivateMessage(
        ...
        // 回复模式：返回创建的 Post 对象
        final newPost = await NodeSeekClient().createReply(
