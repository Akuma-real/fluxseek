...
class GalleryInfo {
  ...
  static GalleryInfo fromHtml(String html) {
    ...
    for (final anchor in lightboxLinks) {
      ...
      if (thumbnailUrl != null) {
        ...
        final thumbOriginal = ContentImageUtils.getOriginalUrl(thumbnailUrl);
  ...
  static GalleryInfo fromImages(
    ...
    for (var i = 0; i < images.length; i++) {
      ...
      // 同时添加原图 URL 作为 key
      final originalUrl = ContentImageUtils.getOriginalUrl(url);
      if (originalUrl != url) {
  ...
  int? findIndex(String imageUrl) {
    ...
    // 3. 尝试转换为原图 URL 后查找
    final originalUrl = ContentImageUtils.getOriginalUrl(imageUrl);
    if (_thumbnailToIndex.containsKey(originalUrl)) {
      return _thumbnailToIndex[originalUrl];
    }

    // 4. resolvedUrl 转换为原图后查找
    final resolvedOriginalUrl = ContentImageUtils.getOriginalUrl(resolvedUrl);
  ...
  List<String> get heroTags =>
      ContentImageUtils.generateGalleryHeroTags(originalUrls);
...
class ContentImageUtils {
  ...
  static Future<String?> resolveUploadUrl(String shortUrl) async {
    ...
    // 调用 API 解析
    try {
      final resolved = await NodeSeekClient().resolveShortUrl(shortUrl);
      _uploadUrlCache[shortUrl] = resolved;
      return resolved;
    } catch (e) {
      debugPrint(
        '[ContentImageUtils] Failed to resolve upload url: $shortUrl, error: $e',
      );
  ...
  /// 将优化图 URL 转换为原图 URL
  ///
  /// 优化图路径: .../uploads/default/optimized/4X/7/5/c/75c...dc_2_690x270.png
