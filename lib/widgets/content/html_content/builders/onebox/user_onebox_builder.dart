...
Widget buildUserOnebox({
  ...
  final userInfo = usernameLink == null
      ? null
      : NodeSeekUrlParser.parseUser(usernameLink.attributes['href'] ?? '');
  // 从 @username 提取 username
  final usernameFromText = usernameText.startsWith('@')
      ? usernameText.substring(1)
      : usernameText;
  final username = userInfo?.username ?? usernameFromText;
    ...
    onTap: username.isNotEmpty
        ? () {
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) =>
                    UserProfilePage(username: username, uid: userInfo?.uid),
              ),
            );
          }
        : null,
