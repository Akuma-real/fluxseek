...
List<GridImageData> extractGridImages(dynamic element) {
  ...
  for (final img in imgElements) {
    ...
    if (!ContentImageUtils.isUploadUrl(src)) {
      src = UrlHelper.resolveUrlWithCdn(src);
    }

    // 尝试获取原图链接
    String? fullSrc = ContentImageUtils.findOriginalImageUrl(img);
    if (fullSrc != null && !ContentImageUtils.isUploadUrl(fullSrc)) {
    ...
    images.add(
      GridImageData(
        ...
        fullSrc:
            fullSrc ??
            (ContentImageUtils.isUploadUrl(src)
                ? src
                : ContentImageUtils.getOriginalUrl(src)),
        width: width,
        height: height,
      ),
    );
...
class _GridImageTileState extends State<_GridImageTile> {
  ...
  Widget build(BuildContext context) {
    ...
    // 已进入视口：加载图片
    // 检查是否是 upload:// 短链接
    if (!ContentImageUtils.isUploadUrl(widget.imageData.src)) {
      // 普通 URL，直接渲染
    ...
    // upload:// 短链接：检查缓存
    if (ContentImageUtils.isUploadUrlCached(widget.imageData.src)) {
      final resolvedUrl = ContentImageUtils.getCachedUploadUrl(
        widget.imageData.src,
      );
      if (resolvedUrl != null) {
        return _buildImageWidget(
    ...
    return FutureBuilder<String?>(
      future: ContentImageUtils.resolveUploadUrl(widget.imageData.src),
      ...
      child: ClipRRect(
        ...
        child: GestureDetector(
          ...
          child: Hero(
            tag: widget.heroTag,
            child: Image(
              image: appImageProvider(displayUrl),
              fit: BoxFit.cover,
              width: widget.columnWidth,
              height: displayHeight,
  ...
  void _openViewer(BuildContext context, String resolvedFullUrl) {
    ...
    final resolvedGalleryImages = widget.gridOriginalImages
        .map((url) => ContentImageUtils.getOriginalUrl(url))
        .toList();
    // 当前点击的图片使用解析后的 URL
    if (widget.index >= 0 && widget.index < resolvedGalleryImages.length) {
      resolvedGalleryImages[widget.index] = ContentImageUtils.getOriginalUrl(
        resolvedFullUrl,
      );
    }

    ContentImageUtils.openViewer(
      context: context,
      imageUrl: ContentImageUtils.getOriginalUrl(resolvedFullUrl),
