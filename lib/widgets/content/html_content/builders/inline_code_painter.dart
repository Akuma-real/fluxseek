...
class InlineCodePainter extends CustomPainter {
  ...
  InlineCodePainter({required this.groups, required this.isDark})
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final group in groups) {
      ...
      for (int i = 0; i < rowCount; i++) {
        ...
        final Radius leftRadius = isFirst
            ? const Radius.circular(radius)
            : Radius.zero;
        final Radius rightRadius = isLast
            ? const Radius.circular(radius)
            : Radius.zero;
  ...
  bool shouldRepaint(InlineCodePainter oldDelegate) {
    return isDark != oldDelegate.isDark ||
        _groupsHash != oldDelegate._groupsHash;
  }
}
