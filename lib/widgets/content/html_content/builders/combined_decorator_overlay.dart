...
class CombinedDecoratorOverlay extends StatefulWidget {
  ...
  @override
  State<CombinedDecoratorOverlay> createState() =>
      _CombinedDecoratorOverlayState();
}
...
class _CombinedDecoratorOverlayState extends State<CombinedDecoratorOverlay>
  ...
  void _scan() {
    ...
    void visitRenderObject(RenderObject ro, Offset parentOffset) {
      if (ro is RenderScanBoundary) return;

      if (ro is RenderParagraph) {
        _extractFromParagraph(
          ro,
          parentOffset,
          paragraphIndex,
          newCodeGroups,
          ...
          try {
            final boxes = paragraph.getBoxesForSelection(
              TextSelection(
                ...
                extentOffset: charIndex + textLength,
              ),
            );
  ...
  List<SpoilerGroup> _buildSpoilerGroups(List<_TempRect> tempRects) {
    ...
    for (final temp in tempRects) {
      groupMap
          .putIfAbsent(temp.groupId, () => [])
          .add(SpoilerRect(rect: temp.rect));
    }
