...
/// 构建 内容图片轮播 (d-image-grid mode=carousel)
...
class _ImageCarouselState extends State<_ImageCarousel> {
  ...
  void _initResolvedUrls() {
    for (int i = 0; i < widget.images.length; i++) {
      ...
      if (ContentImageUtils.isUploadUrl(src)) {
        if (ContentImageUtils.isUploadUrlCached(src)) {
          final cached = ContentImageUtils.getCachedUploadUrl(src);
  ...
  Future<void> _resolveUploadUrls() async {
    ...
    for (final i in pending) {
      if (!mounted) return;
      final src = widget.images[i].src;
      final resolved = await ContentImageUtils.resolveUploadUrl(src);
  ...
  void _openViewer(
    ...
    if (globalIndex >= 0 && globalIndex < resolvedGalleryImages.length) {
      resolvedGalleryImages[globalIndex] = ContentImageUtils.getOriginalUrl(
        resolvedFullUrl,
      );
    }
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Hero(
        tag: heroTag,
        child: Image(
          image: appImageProvider(url, maxHeight: maxHeight),
          fit: BoxFit.contain,
