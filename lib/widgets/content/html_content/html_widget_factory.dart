...
class HtmlWidgetFactory extends WidgetFactory {
  ...
  Widget? buildImage(BuildTree tree, ImageMetadata data) {
    ...
    // 独立大表情：消息仅包含 emoji 时服务端会添加 only-emoji class
    final bool isOnlyEmoji =
        isEmoji && tree.element.classes.contains('only-emoji');

    // NodeSeek 贴图（:xhj003: / :ac01: / :yct015: / :emoji00:）
    // 网页端 CSS：img.sticker { max-width: 90px; vertical-align: middle; }
    final bool isSticker = tree.element.classes.contains('sticker');
    ...
    // NodeSeek 贴图独立分支：限制 max-width 90dp，保持 inline 显示
    if (isSticker) {
      return _buildStickerWidget(url);
    }

    // 普通 URL：直接构建 widget，无需 FutureBuilder
    if (!ContentImageUtils.isUploadUrl(url)) {
    ...
    // upload:// 短链接：检查缓存
    if (ContentImageUtils.isUploadUrlCached(url)) {
      final resolvedUrl = ContentImageUtils.getCachedUploadUrl(url);
      if (resolvedUrl != null) {
        return _buildImageWidget(
          resolvedUrl,
          url,
    ...
    return FutureBuilder<String?>(
      future: ContentImageUtils.resolveUploadUrl(url),
  ...
  /// 构建 NodeSeek 贴图 widget
  ///
  /// 对齐网页端 CSS：`img.sticker { max-width: 90px; vertical-align: middle; }`。
  /// 用 `InlineCustomWidget` 包裹，使贴图跟文字保持行内排版。
  Widget _buildStickerWidget(String url) {
    final resolved = UrlHelper.resolveUrlWithCdn(url);
    return InlineCustomWidget(
      alignment: PlaceholderAlignment.middle,
      child: ConstrainedBox(
        constraints: const BoxConstraints(maxWidth: 90, maxHeight: 90),
        child: Image(
          image: stickerImageProvider(resolved),
          fit: BoxFit.contain,
    ...
    return Builder(
      builder: (context) {
        ...
        final double emojiBaseSize =
            ...
            16.0;
        // only-emoji: 独立大表情 32dp（CSS: img.emoji.only-emoji { width: 32px; height: 32px }）
        // 普通 emoji: 1em
        ...
        if (!isGalleryImage || isEmoji) {
          ...
          if (isEmoji) {
            Widget emojiWidget = Container(
              // only-emoji: 独立大表情添加垂直间距（CSS: margin: .5em 0）
              margin: isOnlyEmoji
        ...
        Widget buildGalleryImage() {
          ...
          if (isSvg && resolvedUrl != null) {
            ...
            return GestureDetector(
              onTap: () {
                ContentImageUtils.openViewerFiltered(
                  context: context,
                  galleryInfo: galleryInfo!,
                  revealedImageUrls: revealedImageUrls,
  ...
  /// Future 缓存和 HtmlWidgetFactory 实例同生命周期（跟随 HtmlContent State）。
