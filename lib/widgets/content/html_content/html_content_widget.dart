...
/// HTML 内容渲染 Widget
/// 封装了所有自定义渲染逻辑
class HtmlContent extends ConsumerStatefulWidget {
  final String html;
  final TextStyle? textStyle;
  final bool compact; // 紧凑模式：移除段落边距
  /// 内部链接点击回调 (www.nodeseek.com 话题链接)
  ...
  static void preloadPangu(List<String> htmlList) {
    _HtmlContentState._preloadPangu(htmlList);
  }
...
class _HtmlContentState extends ConsumerState<HtmlContent> {
  late final HtmlWidgetFactory _widgetFactory;
  ...
  void initState() {
    ...
    _widgetFactory = HtmlWidgetFactory(
      context: context,
  ...
  void _trackClick(String url) {
    ...
    if (NodeSeekUrlParser.isUserLink(url)) return;
    // 2. 附件/上传链接
  ...
  Widget build(BuildContext context) {
    ...
    final htmlWidget = HtmlWidget(
      ...
      customStylesBuilder: (element) {
        ...
        // img 垂直居中（与网页端一致：img { vertical-align: middle }）
  ...
  Widget? _buildCustomWidget(BuildContext context, dynamic element) {
    ...
    // 处理懒加载视频 (div.lazy-video-container)
    ...
    // 每个 HtmlContent 自带独立的 CombinedDecoratorOverlay
