...
class LinkInsertDialog extends StatefulWidget {
  ...
  const LinkInsertDialog({super.key, this.initialText, this.initialUrl});
...
class _LinkInsertDialogState extends State<LinkInsertDialog> {
  ...
  void _submit() {
    if (_formKey.currentState!.validate()) {
      Navigator.of(
        context,
      ).pop({'text': _textController.text, 'url': _urlController.text});
    }
  }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        ...
        FilledButton(onPressed: _submit, child: Text(S.current.common_confirm)),
      ],
    );
  }
}
...
Future<Map<String, String>?> showLinkInsertDialog(
  ...
  return showAppDialog<Map<String, String>>(
    context: context,
    builder: (context) =>
        LinkInsertDialog(initialText: initialText, initialUrl: initialUrl),
  );
}
