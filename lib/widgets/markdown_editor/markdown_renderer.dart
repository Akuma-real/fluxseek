...
import '../content/html_content/html_content.dart';
...
class MarkdownBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 3. 预处理 内容图片格式 (![alt|WxH](url) -> HTML img)
    processedData = _processAppImages(processedData);
    ...
    // 11. 使用 HtmlContent 渲染，与帖子显示保持一致
    return HtmlContent(
      html: html,
  ...
  /// 处理 内容图片格式：![alt|widthxheight](url) -> <img src="" width="" height="" alt="">
  /// 标准 Markdown 包不识别竖线语法，需要手动转换
  String _processAppImages(String text) {
    // 匹配 ![alt|WxH](url) 格式
    final sizedImageRegex = RegExp(
      r'!\[([^\]|]*)\|(\d+)x(\d+)\]\(([^)\s]+)\)',
    );

    return text.replaceAllMapped(sizedImageRegex, (match) {
  ...
  String _restoreGridBlocks(String html, Map<String, String> gridBlocks) {
    ...
    for (final entry in gridBlocks.entries) {
      ...
      // 预处理 内容图片格式（grid 内也可能包含 ![alt|WxH](url)）
      markdownContent = _processAppImages(markdownContent);
  ...
  String _restoreQuoteBlocks(String html, Map<String, String> quoteBlocks) {
    ...
    for (final entry in quoteBlocks.entries) {
      ...
      // 构建 aside.quote HTML（与网页端输出的格式一致）
