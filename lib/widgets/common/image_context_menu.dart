...
import '../content/html_content/image_utils.dart';
...
class ImageContextMenu {
  ...
  static Future<void> _copyImage(String imageUrl) async {
    try {
      final bytes = await AppCacheManager().getImageBytes(imageUrl);
  ...
  static Future<void> _shareImage(String imageUrl) async {
    try {
      final file = await AppCacheManager().getSingleFile(imageUrl);
