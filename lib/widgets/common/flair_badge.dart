...
class FlairBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // SVG 图片使用 AppCacheManager 下载后用 jovial_svg 渲染
    if (_isSvg) {
    ...
    return Tooltip(
      ...
      child: Container(
        ...
        child: Center(
          child: Image(
            image: appImageProvider(fullUrl),
            width: imageSize,
            height: imageSize,
            fit: BoxFit.contain,
            errorBuilder: (_, _, _) {
...
/// SVG Flair 徽章组件（使用 AppCacheManager 加载）
...
class _SvgFlairBadgeState extends State<_SvgFlairBadge> {
  ...
  Future<void> _loadSvg() async {
    ...
    try {
      final file = await AppCacheManager().getSingleFile(widget.url);
