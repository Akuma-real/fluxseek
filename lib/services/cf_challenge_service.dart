...
class CfChallengeService {
  ...
  Future<bool?> showManualVerify(
    ...
    final effectiveVerifyUrl = verifyUrl ?? AppConstants.baseUrl;
    CfChallengeLogger.logVerifyStart(effectiveVerifyUrl);
    unawaited(
      CfChallengeLogger.logAccessIps(
        url: effectiveVerifyUrl,
...
class _CfChallengePageState extends State<CfChallengePage> {
  ...
  Future<void> _syncLiveCookiesToCookieJar() async {
    ...
    await _persistReadableCfClearance();
  }

  /// Linux WPE 有时 CookieManager 能读到 HttpOnly cf_clearance，
  /// 但 getCookies(url) 边界同步拿不到可写入 CookieJar 的条目。
  /// 这里用已读到的值做最后兜底，避免验证成功后请求层仍无 cookie。
  Future<void> _persistReadableCfClearance() async {
    final synced = await CookieJarService().getCfClearance();
    if (synced != null && synced.isNotEmpty) return;
    ...
    await CookieJarService().setCookie(
      'cf_clearance',
      cookieValue,
      url: widget.verifyUrl,
      path: '/',
      secure: true,
      httpOnly: true,
    );
    ...
    _noChallengeCheckTimer = Timer(_noChallengeCheckDelay, () async {
      ...
      try {
        ...
        if (cookieValue != null &&
          ...
          // 已确认来自 CF Challenge 的请求，不要因为页面 HTML 结构未命中
          // 本地特征检测就提前判失败。Linux WPE 上 Cloudflare 可能在
          // onPageCommitVisible 后快速改写 DOM，继续轮询到超时更稳。
          if (widget.knownChallenge) {
            debugPrint('[CfChallenge] 页面暂未识别到盾且无新 cf_clearance，继续等待');
            CfChallengeLogger.log(
              '[VERIFY] No challenge marker yet, keep waiting',
            );
            if (_isBackground) {
              _promoteToForeground();
            }
          ...
          // 手动入口没有明确 CF 403 证据时，仍保留快速退出，避免非验证页死等。
