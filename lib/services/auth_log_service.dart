...
class AuthLogService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _logger = Logger(
        ...
        output: MultiOutput([if (kDebugMode) ConsoleOutput(), fileOutput]),
  ...
  Future<void> logAuthInvalid({
    ...
    final message =
        '[AUTH_INVALID] source=$source reason=$reason${extra != null ? ' extra=$extra' : ''}';
    _logger!.w(message);
  }
  ...
  Future<void> logWebViewVerify({
    ...
    final message =
        '[WEBVIEW_VERIFY] success=$success${reason != null ? ' reason=$reason' : ''}${extra != null ? ' extra=$extra' : ''}';
    if (success) {
      _logger!.i(message);
    } else {
      _logger!.w(message);
  ...
  Future<void> log(String event, {Map<String, dynamic>? details}) async {
    ...
    final message =
        '[AUTH] $event${details != null ? ' details=$details' : ''}';
    _logger!.i(message);
  }
