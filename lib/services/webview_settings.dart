...
(function(){
  if(window.__fluxseekScrollFix) return;
  window.__fluxseekScrollFix = true;
  window.addEventListener('wheel', function(e){ e.preventDefault(); }, {passive:false});
  window.__fluxseekScroll = function(dx, dy) {
...
''';
  ...
  ///
  /// DoH 代理移除后不再需要自签 CA 信任，保留方法签名以避免破坏调用点。
  ...
  /// iOS 15 及以下缺少的现代 JS API polyfill，在页面最早期注入以兼容 网页端
...
''',
  ...
  static InAppWebViewSettings get visible => InAppWebViewSettings(
    ...
    // 登录和验证页不要复用旧 challenge/Turnstile 资源，否则可能误报设备时间。
    cacheEnabled: false,
  ...
  void _doScroll(double dx, double dy) {
    ...
    if (controller != null && (dx != 0 || dy != 0)) {
      controller.evaluateJavascript(
        source: 'window.__fluxseekScroll?.($dx,$dy)',
      );
    }
  }
