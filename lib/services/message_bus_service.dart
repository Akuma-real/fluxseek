...
/// MessageBus 长轮询客户端（Discourse MessageBus 协议遗留）
///
/// NodeSeek 无对应 API，当前实现暂未接入实际业务，保留供未来扩展或兼容分叉站点。
class MessageBusService {
  ...
  // 对齐网页端 message-bus minPollInterval (100ms)
  ...
  // MessageBus 独立域名配置
  String? _baseUrl; // 独立域名（如 https://www.nodeseek.com），null 表示用主站
  ...
  Future<void> _poll(int generation) async {
    while (!_shouldStop &&
      ...
      try {
        ...
        // 后台模式使用更长的轮询间隔（对齐网页端 backgroundCallbackInterval）
        if (_backgroundMode) {
        ...
        final response = await _dio.post<ResponseBody>(
          ...
          options: Options(
            ...
            extra: {'isSilent': true, 'skipCsrf': true},
          ),
        );
