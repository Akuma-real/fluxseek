...
class UploadResult {
  ...
  /// 生成 NodeSeek 兼容格式的 Markdown 图片语法
...
/// 上传相关
mixin _UploadsMixin on _NodeSeekClientBase {
  /// 获取图片请求头
  ...
  Future<Uint8List?> downloadImage(String url) async {
    try {
      ...
      if (response.data is! List<int>) {
        debugPrint(
          '[NodeSeekClient] Invalid response data type for image: $url',
        );
        return null;
      }
      ...
      if (bytes.isEmpty) {
        debugPrint('[NodeSeekClient] Empty image data: $url');
        return null;
      }
      ...
      if (contentType != null && !contentType.startsWith('image/')) {
        debugPrint(
          '[NodeSeekClient] Invalid content-type for image: $contentType, url: $url',
        );
        return null;
      }
      ...
      if (!_isValidImageData(bytes)) {
        debugPrint(
          '[NodeSeekClient] Invalid image data (magic bytes check failed): $url',
        );
        return null;
      }
      ...
      debugPrint('[NodeSeekClient] Download image failed: $e, url: $url');
      return null;
    }
  }
    ...
    for (int attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        final response = await _dio.post(
          ...
          data: formData,
          options: Options(
            extra: {'showErrorToast': attempt >= maxRetries},
          ), // 仅最后一次尝试才弹 toast
        );
        ...
        debugPrint('[NodeSeekClient] Upload image failed: $e');
        ...
        if (innerError is RateLimitException && attempt < maxRetries) {
          ...
          debugPrint(
            '[NodeSeekClient] 速率限制，等待 ${waitSeconds}s 后重试 '
  ...
  Future<List<Map<String, dynamic>>> lookupUrls(List<String> shortUrls) async {
    ...
    try {
      ...
      debugPrint('[NodeSeekClient] lookupUrls failed: $e');
      return [];
    }
  }
