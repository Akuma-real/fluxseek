...
/// 4. Linux/WPE 显式传递时区，避免 Turnstile 误判设备时间。
class WindowsWebViewEnvironmentService {
  ...
  bool get _isSupported => !kIsWeb && (Platform.isWindows || Platform.isLinux);

  bool get _supportsProxyEnvironment => !kIsWeb && Platform.isWindows;
  ...
  Future<void> setProxy(String? proxyUrl) async {
    if (!_supportsProxyEnvironment) return;
  ...
  Future<void> _createEnvironment() async {
    try {
      if (Platform.isLinux) {
        final timezone = await _detectLocalTimezone();
        _environment = await WebViewEnvironment.create(
          settings: WebViewEnvironmentSettings(
            timeZoneOverride: timezone,
            preferredLanguages: const ['zh-CN', 'zh', 'en-US', 'en'],
          ),
        );
        ...
        debugPrint(
          '[WebViewEnv] Linux WPE environment initialized: '
          'timeZoneOverride=$timezone',
        );
        return;
      }
  ...
  Future<String> _detectLocalTimezone() async {
    try {
      ...
      if (timezone.identifier.trim().isNotEmpty) return timezone.identifier;
    } catch (e) {
      debugPrint('[WebViewEnv] detect timezone failed: $e');
    }
