...
class CookieJarService {
  ...
  /// 可配置的关键 cookie 名集合。
  ///
  /// NodeSeek 当前站点登录态以 `session` 为主，`smac` / `hmti_` /
  /// `fog` 为同站辅助 cookie。`_t` / `_forum_session` 仅保留旧兼容。
  ...
  /// 获取会话 token（优先 NodeSeek 的 `session` cookie，回退到旧 `_t`）
  Future<String?> getTToken() async {
    // NodeSeek 主 session cookie
    final session = await getCookieValue('session');
    if (session != null && session.isNotEmpty) return session;
    // 旧 Discourse 兼容
    return getCookieValue('_t');
  }
  ...
  Future<Map<String, dynamic>> getTTokenDiagnostics() async {
    ...
    try {
      final uri = Uri.parse(AppConstants.baseUrl);
      final cookies = await _cookieJar!.loadForRequest(uri);
      final tCookies = cookies
          .where((c) => c.name == 'session' || c.name == '_t')
          .toList();
      return {
