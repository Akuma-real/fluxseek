...
class AndroidCdpService {
  ...
  Future<bool> awaitTargetReady({
    ...
    try {
      ...
      if (!ready) {
        AppLogger.warning('Native CDP target not ready', tag: _tag);
      }
      return ready;
    } on PlatformException catch (e, stackTrace) {
      _logPlatformError(
        ...
        e,
        stackTrace,
        extras: {'timeoutMs': timeout.inMilliseconds},
      );
      return false;
    } catch (e, stackTrace) {
      _logUnexpectedError(
  ...
  Future<Map<String, dynamic>?> getCookies(List<String> urls) async {
    ...
    try {
      ...
      _lastGetCookiesResult = result == null
          ? null
  ...
  Future<Map<String, dynamic>?> setCookie(Map<String, dynamic> params) async {
    ...
    try {
      ...
      _logPlatformError(
        ...
        e,
        stackTrace,
        extras: {'cookieName': params['name'], 'url': params['url']},
      );
      rethrow;
    } catch (e, stackTrace) {
      _logUnexpectedError(
  ...
  Future<Map<String, dynamic>?> deleteCookies(
