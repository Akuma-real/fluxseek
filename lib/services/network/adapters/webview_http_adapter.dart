...
class WebViewHttpAdapter implements HttpClientAdapter {
  ...
  Future<void> initialize() async {
    ...
    try {
      _headlessWebView = HeadlessInAppWebView(
        // 桌面端共享同一个 WebView 环境，确保 cookie/时区等上下文一致。
        webViewEnvironment: (Platform.isWindows || Platform.isLinux)
  ...
  Future<ResponseBody> fetch(
    ...
    if (shouldSyncAppCookies) {
      ...
      final repaired = await _repairDuplicatedSessionCookiesIfNeeded(
        requestUri,
      );
  ...
  Future<String?> _buildStreamedBodyScript(
    ...
    try {
      ...
      await controller.postWebMessage(
        message: WebMessage(
          data: '__fluxseek:body:$requestId',
          ports: [channel.port2],
        ),
      ...
      return 'fetchOptions.body = window.__fluxseekTakeRequestBody(${jsonEncode(requestId)});';
    } catch (e) {
      if (!transferStarted) {
        debugPrint(
  ...
  Future<void> _installRequestBodyBridge(
    ...
    await controller.evaluateJavascript(
      source: '''
        (function() {
          if (window.__fluxseekRequestBodyBridgeInstalled) return;
          window.__fluxseekRequestBodyBridgeInstalled = true;
          window.__fluxseekRequestBodyTransfers = new Map();

          function ensureState(requestId) {
            var state = window.__fluxseekRequestBodyTransfers.get(requestId);
            if (!state) {
              state = {
                chunks: [],
                body: null
              };
              window.__fluxseekRequestBodyTransfers.set(requestId, state);
            }
            return state;
          }

          window.__fluxseekTakeRequestBody = function(requestId) {
            var state = window.__fluxseekRequestBodyTransfers.get(requestId);
            if (!state || state.body === null) {
              throw new Error('Request body not ready for request ' + requestId);
            }
            var body = state.body;
            window.__fluxseekRequestBodyTransfers.delete(requestId);
            return body;
          };

          window.addEventListener('message', function(event) {
            if (typeof event.data !== 'string' || !event.data.startsWith('__fluxseek:body:')) {
              return;
            }

            var requestId = event.data.substring('__fluxseek:body:'.length);
  ...
  Future<void> _runStartupSessionCookieSelfCheck({
    ...
    LogWriter.instance.write({
      ...
      'message': repaired
          ? '启动时重复 session cookie 已修复'
          : '启动时重复 session cookie 修复失败',
      'reason': reason,
      'url': requestUri.toString(),
      'duplicateNames': duplicateNames,
      'repaired': repaired,
  ...
  Future<bool> _repairDuplicatedSessionCookiesIfNeeded(Uri requestUri) async {
    ...
    if (lastRepairAt != null &&
        DateTime.now().difference(lastRepairAt) < const Duration(seconds: 2)) {
      return false;
    }
  ...
  Future<bool> _repairDuplicatedSessionCookies(
    ...
    final canonicalCookies = await jar.loadCanonicalCookiesForRequest(
      requestUri,
    );
    ...
    for (final name in affectedNames) {
      ...
      final canonical =
          canonicalByName[name] ??
          _selectCanonicalCookieFromWebView(duplicates[name]!, requestUri);
      if (canonical == null) {
        continue;
      }
