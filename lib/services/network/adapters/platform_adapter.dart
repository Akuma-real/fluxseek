...
HttpClientAdapter createExternalHttpAdapter() {
  ...
  final adapter = _DynamicAdapter(settings, proxySettings, fallbackService);
...
class _GatewayAdapterWrapper implements HttpClientAdapter {
  ...
  static final WebViewHttpAdapter _sharedWebViewAdapter = WebViewHttpAdapter();

  WebViewHttpAdapter _getWebViewAdapter() {
    return _sharedWebViewAdapter;
  }
  ...
  void close({bool force = false}) {
    // WebView 初始化在 Linux/Android 上开销很高。多个 Dio 实例会共享同一个
    // adapter，单个 Dio 关闭时不要销毁共享 WebView，会在进程退出时自然释放。
  ...
  bool _shouldUseWebView(RequestOptions options) {
    ...
    final forceWebView = options.extra['forceWebViewAdapter'] == true;
    if (!forceWebView &&
        !WebViewAdapterSettingsService.instance.shouldUseWebView(uri)) {
      return false;
    }
    if (forceWebView && uri.host != Uri.parse(AppConstants.baseUrl).host) {
      return false;
    }
    ...
    if (!forceWebView && explicitlyHtml) {
      return false;
    }
    final apiLikeGet =
    ...
    if (!forceWebView && (method == 'GET' || method == 'HEAD') && !apiLikeGet) {
      return false;
    }
...
/// iOS/macOS 在 network ↔ native（Cupertino）之间切换；
/// Windows 在 network ↔ native（Dio IO）之间切换。
