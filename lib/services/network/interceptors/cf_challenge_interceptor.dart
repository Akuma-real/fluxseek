...
import '../adapters/webview_http_adapter.dart';
import '../../../l10n/s.dart';
...
class CfChallengeInterceptor extends Interceptor {
  ...
  /// 仅靠响应体文本匹配容易误判（用户帖子含关键词、站点自身 403 等），
  ...
  static bool _isCfChallengeResponse(Response? response) {
    ...
    // 2. Content-Type 必须是 HTML（站点自身的 403 返回 JSON）
    ...
    if (statusCode == 403 &&
      ...
      if (result == true) {
        ...
        try {
          ...
          retryOptions.extra['forceWebViewAdapter'] = true;
          ...
          final retryDio = Dio();
          final webViewAdapter = WebViewHttpAdapter();
          retryDio.httpClientAdapter = webViewAdapter;
          Response<dynamic> response;
          try {
            response = await retryDio.fetch(retryOptions);
          } finally {
            webViewAdapter.close(force: false);
            retryDio.close(force: false);
          }
