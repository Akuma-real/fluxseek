...
> 本文档描述 fluxseek 的 Cookie 管理架构。

## 版本历史

| 阶段 | 模型 | 问题 |
|------|------|------|
| 早期 | WebView ↔ CookieJar 双向常态同步 | 多套状态互相污染；cookie 属性在跨平台同步中丢失 |
| 中期 | 浏览器优先 + SessionSnapshot | 去掉常态同步，但保留了复杂的 strategy/coordinator 层 |
| 过渡 | 边界同步 + BrowserSessionService | 进一步收敛同步入口，但 hostOnly 推断错误导致多副本 bug |
| **当前** | **CookieJar 唯一存储 + 原始头队列** | 删除 strategy/coordinator/snapshot，storageKey 放宽 |
...
| `cookie_relaxed_key_v4` | 当前 | storageKey 放宽，清理旧 cookie |
